...
# oco alg
# d(r, s) = max_{w in B_2}(<w, r_t> - h_s(w)), B_2: 2-norm unit ball
# h_s(w) := max_{s in S} <w,s>, S: ε 1-norm ball; i.e. the dual norm of w
# loss func f_t(w) = - d(r, S) = - (w ⋅ r_t - h_s(w))
# -: concave -> convex
function shimkin_step(t, w::Vector{Vector{Float64}}, # current iterate
    ...
    #w_vector = EG_pm_vector(t, reduce(vcat, loss), reduce(vcat, w), reduce(vcat, ∇f_t), η, R)
...
function ∇h(w::Vector{Vector{Float64}}, R::Float64 # radius of approach set S (ε_1-norm ball)
    ...
    for i in 1:length(w)
        g[i] = zeros(size(w[i]))
        val , idx = findmax(abs.(w[i])) # index of the maximum abs value of w
        if val > max_w
            max_w = val
            index = i
        end
    end
    ~, idx = findmax(abs.(w[index]))
    g[index][idx] = R * sign(w[index][idx])
    return g 
end

# 將oco問題獨立出來 驗algorithm結果是否為no regret
# W = B_2, 2-norm unit ball
function ogd(η, ∇f_t, w, r_t, t, R)
    #η = D * sqrt(1) / sqrt(g_dual^2) #Orabona Chp 6
    η /= sqrt(t) #might change the global value of η, should be avoided, Zinkevich 2003

    w -= η * ∇f_t
    
    # project back to constraint set (2-norm ball)
    w_proj = w ./ max(1,norm(w, 2))

    return w_proj
end

#comparator: summation(cumulative loss)取maximum

# input s =  w_{t-1}
# y_hat = (w_t_plus - w_t_minus) ⋅ loss_t
function EG_pm_vector(t, r_t, w, # current iterate 
    ∇f_t, # gradient 
    η, # learning rate
    R # radius of the \ell_1-norm ball 
    )::Vector{Float64}
